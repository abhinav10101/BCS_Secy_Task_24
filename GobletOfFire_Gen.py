...
class TriwizardMaze(gym.Env):
    ...
    def step(self, action_agent):
        ...
        if tuple(new_agent_pos) not in blocked_cells:
            ...
            for action in range(4):
                ...
                if tuple(candidate_pos) not in blocked_cells:
...
def train(epsilon_start, alpha, gamma, episodes):
    ...
    for episode in range(episodes):
        ...
        while not terminated and not truncated:
            ...
            next_death_state = agent.get_state(next_obs["DeathEater"])
            next_target_state = agent.get_state(next_obs["target"])
            
            agent.update(state, death_state, target_state, action, reward,
                         next_state, next_death_state, next_target_state,
                         alpha, gamma)
            
            observation = next_obs
            if(info["Loss"] or truncated):
                    count = 0
            elif(info["Win"]): 
                count += 1
